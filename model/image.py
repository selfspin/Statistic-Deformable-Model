...
class ImgModel(nn.Module):
    def __init__(self, img_shape, pca_mean, pca_features, point_shape_factor=1, device='cuda'):
        super().__init__()
        self.device = device
        ...
        # 控制点位移PCA
        self.pca_mean = pca_mean.to(self.device).float()
        self.pca_features = pca_features.to(self.device).float()
        # PCA特征系数
        self.coefficient = nn.Parameter(torch.zeros([self.pca_features.shape[0]]))

        # 使用四元数表示旋转
        self.quaternion = nn.Parameter(torch.Tensor([1, 0, 0, 0]))
        # 平移
        self.bias = nn.Parameter(torch.Tensor([0, 0, 0]))

    def forward(self, image_from):
        # 图像对应控制点 (1, 3, x, y, z)
        control_points = self.coefficient @ self.pca_features + self.pca_mean
        control_points = control_points.reshape([3, self.point_shape[0], self.point_shape[1], self.point_shape[2]])[
            None]
        control_points += self.control_points_origin_position[None]
        # (1, x, y, z, 3)
        ...
        # 进行全局的旋转平移
        rotation_matrix = self.as_rotation_matrix(self.quaternion)
        # (1, x, y, z, 3)
        transformed_point = transformed_point.permute(0, 2, 3, 4, 1) @ rotation_matrix + self.bias
        for i in range(3):
